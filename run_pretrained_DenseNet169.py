...
directory_path = '/home/hwi3319/COMP_SCI_496_SML'
...
class Dataset(data.Dataset):
    def __init__(self, img, lb, pretrained_models):
        self.labels = lb
        self.images = img
        self.pretrained_models = list(pretrained_models.children())[:-1]#.cuda() (1)
        self.pretrained_models.append(nn.AvgPool2d(7)) #(2)
        self.pretrained_models = nn.Sequential(*self.pretrained_models).to(device) #(3)
...
# use DenseNet121 pre-trained
training_set = Dataset(img_train, lb_train, models.densenet169(pretrained=True))
training_generator = data.DataLoader(training_set, **params)

validation_set = Dataset(img_val, lb_val, models.densenet169(pretrained=True))
validation_generator = data.DataLoader(validation_set, **params)

test_set = Dataset(img_test, lb_test, models.densenet169(pretrained=True))
...
for e in range(epochs):
    ...
    torch.save(model.state_dict(), directory_path + '/densenet169' + str(e) + '.ckpt')
...
pd.DataFrame(roc).to_csv('roc_densenet169_normalized.csv')
pd.DataFrame(sensitivity).to_csv('sensitivity_densenet169_normalized.csv')
pd.DataFrame(specificity).to_csv('specificity_densenet169_normalized.csv')
pd.DataFrame(accuracy).to_csv('accuracy_densenet169_normalized.csv')