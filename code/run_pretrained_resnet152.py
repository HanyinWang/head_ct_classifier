...
training_set = Dataset(img_train, lb_train, models.resnet152(pretrained=True))
training_generator = data.DataLoader(training_set, **params)

validation_set = Dataset(img_val, lb_val, models.resnet152(pretrained=True))
validation_generator = data.DataLoader(validation_set, **params)

test_set = Dataset(img_test, lb_test, models.resnet152(pretrained=True))
...
pd.DataFrame(roc).to_csv(output_dir + 'roc_resnet152_normalize.csv')
pd.DataFrame(sensitivity).to_csv(output_dir + 'sensitivity_resnet152_normalize.csv')
pd.DataFrame(specificity).to_csv(output_dir + 'specificity_resnet152_normalize.csv')
pd.DataFrame(accuracy).to_csv(output_dir + 'accuracy_resnet152_normalize.csv')
