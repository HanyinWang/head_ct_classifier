...
class Dataset(data.Dataset):
    def __init__(self, img, lb, pretrained_models):
        self.labels = lb
        self.images = img
        self.pretrained_models = nn.Sequential(*list(pretrained_models.children())[:-1]).cuda()
    ...
    def __getitem__(self, index):
        ...
        _scan = np.empty([28,3,299,299])
        _scan = torch.from_numpy(_scan).float()
        for i in range(28):
            _scan[i, :, :, :] = transform(scan[i, :, :, :])
        X = self.pretrained_models(_scan.cuda()).flatten()
...
training_set = Dataset(img_train, lb_train, models.inception_v3(pretrained=True))
training_generator = data.DataLoader(training_set, **params)

validation_set = Dataset(img_val, lb_val, models.inception_v3(pretrained=True))
validation_generator = data.DataLoader(validation_set, **params)

test_set = Dataset(img_test, lb_test, models.inception_v3(pretrained=True))
...
pd.DataFrame(roc).to_csv('roc_inception_v3.csv')
pd.DataFrame(sensitivity).to_csv('sensitivity_inception_v3.csv')
pd.DataFrame(specificity).to_csv('specificity_inception_v3.csv')
pd.DataFrame(accuracy).to_csv('accuracy_inception_v3.csv')